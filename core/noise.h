...
inline CUDA_CALLABLE float dot_grid_gradient_3d_gradient(
        uint32 state, int ix, int iy, int iz, float d_dx, float d_dy, float d_dz) {
    vec3 gradient = random_gradient_3d(state, ix, iy, iz);
    return (d_dx * gradient[0] + d_dy * gradient[1] + d_dz * gradient[2]);
}

inline CUDA_CALLABLE float dot_grid_gradient_4d(
        uint32 state, int ix, int iy, int iz, int it, float dx, float dy, float dz, float dt) {
    vec4 gradient = random_gradient_4d(state, ix, iy, iz, it);
    return (dx * gradient[0] + dy * gradient[1] + dz * gradient[2] + dt * gradient[3]);
}

inline CUDA_CALLABLE float dot_grid_gradient_4d_gradient(
        uint32 state, int ix, int iy, int iz, int it, float d_dx, float d_dy, float d_dz, float d_dt) {
    vec4 gradient = random_gradient_4d(state, ix, iy, iz, it);
...
inline CUDA_CALLABLE float noise_3d(
        uint32 state, int x0, int y0, int z0, int x1, int y1, int z1, float dx, float dy, float dz) {
    // vXYZ
...
inline CUDA_CALLABLE float noise_4d(uint32 state,
                                    int x0,
                                    int y0,
                                    int z0,
                                    int t0,
...
inline CUDA_CALLABLE vec4 noise_4d_gradient(uint32 state,
                                            int x0,
                                            int y0,
                                            int z0,
                                            int t0,
...
inline CUDA_CALLABLE vec3 curlnoise(uint32 state, const vec3& xyz) {
    ...
    return vec3(grad_field_3[1] - grad_field_2[2], grad_field_1[2] - grad_field_3[0],
