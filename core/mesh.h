...
CUDA_CALLABLE inline bool mesh_query_point(
    ...
    while (count) {
        ...
        if (lower.b) {
            ...
            float left_dist_sq = distance_to_aabb_sq(point, vec3(left_lower.x, left_lower.y, left_lower.z),
                                                     vec3(left_upper.x, left_upper.y, left_upper.z));
            float right_dist_sq = distance_to_aabb_sq(point, vec3(right_lower.x, right_lower.y, right_lower.z),
            ...
            if (left_score < right_score) {
                ...
                if (right_dist_sq < min_dist_sq) stack[count++] = right_index;

                if (left_dist_sq < min_dist_sq) stack[count++] = left_index;
            } else {
    ...
    if (secondary_culls > max_secondary_culls) max_secondary_culls = secondary_culls;
...
// returns true if there is a point (strictly) < distance max_dist
CUDA_CALLABLE inline bool mesh_query_point_sign_normal(uint64_t id,
                                                       const vec3& point,
                                                       float max_dist,
                                                       float& inside,
                                                       int& face,
    ...
    while (count) {
        ...
        if (lower.b) {
            ...
            if (length(normal) / (e0_norm_sq + e1_norm_sq + e2_norm_sq) < 1.e-6f) continue;
            vec2 barycentric = closest_point_to_triangle(p, q, r, point);
            float u = barycentric[0];
            float v = barycentric[1];
            float w = 1.f - u - v;
...
CUDA_CALLABLE inline float solid_angle_iterative(uint64_t id, const vec3& p, const float accuracy_sq) {
    ...
    while (count) {
        ...
        if (lower.b) {
            ...
            angle[count - 1] = robust_solid_angle(mesh.points[mesh.indices[leaf_index * 3 + 0]],
                                                  mesh.points[mesh.indices[leaf_index * 3 + 1]],
                                                  mesh.points[mesh.indices[leaf_index * 3 + 2]], p);
...
// returns true if there is a point (strictly) < distance max_dist
CUDA_CALLABLE inline bool mesh_query_point_sign_winding_number(uint64_t id,
                                                               const vec3& point,
                                                               float max_dist,
                                                               float& inside,
                                                               int& face,
...
CUDA_CALLABLE inline bool mesh_query_ray(uint64_t id,
                                         const vec3& start,
                                         const vec3& dir,
                                         float max_t,
                                         float& t,
    ...
    while (count) {
        ...
        float eps = 1.e-3f;
        float t = 0.0f;
        bool hit = intersect_ray_aabb(start, rcp_dir, vec3(lower.x - eps, lower.y - eps, lower.z - eps),
                                      vec3(upper.x + eps, upper.y + eps, upper.z + eps), t);
